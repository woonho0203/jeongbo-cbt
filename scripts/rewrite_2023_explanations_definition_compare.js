...
const REPORT_PATH = path.join(ROOT, 'CBT_2023_해설_정의_오답비교_재작성_보고.md');
const TARGETS = ['exam_2023-1.json', 'exam_2023-2.json', 'exam_2023-3.json'];
...
function isCalc(q) {
  return /(계산|몇\s*(개|회|번|비트|K|M|초|분|시간)|라인수|\bLOC\b|생산성|개월|비트|전송률|페이지|프레임|페이지 참조|주기억장치|스케줄링|평균|PASS|정렬|이진 검색|fan-in|fan-out|카디널리티|차수|튜플의 최대|주소|임계경로|CPM|PERT|해밍|패리티|서브넷|Subnet|FLSM|마스크|FIFO|LRU|HRN|SJF|Worst Fit|선택.*정렬|삽입.*정렬|버블.*정렬|Postfix|후위 순회|Preorder|트리 구조)/i.test(fullText(q));
}
...
function calcProcess(q, ans) {
  ...
  if (/FIFO/i.test(text) && /페이지/.test(text)) {
    const seq = ((q.stem || '').match(/페이지 (?:순서|참조[^:]*):\s*([0-9,\s]+)/) || [])[1];
    if (seq) {
...
const report = [
  '# 2023년 해설 정의·오답비교 재작성 보고',
  '',
  '- 적용 지침: 정보처리기사 CBT 해설 정의·오답비교 작성 지침',
  '- 적용 범위: 2023년 1회, 2회, 3회 전체 문항',
